...
class ActiveUserView(View):
    def get(self, request, active_code):
        all_recods = EmailVerifyRecord.objects.filter(code=active_code)
        if all_recods:
...
class RegisterView(View):
    ...
    def post(self,request):
        ...
        if register_form.is_valid():
            ...
            user_message.message = "欢迎注册JK在线网"
            user_message.save()
...
class MyCourseView(LoginRequiredMixin, View):
    ...
    """
    def get(self, request):
        my_all_course = UserCourse.objects.filter(user=request.user)
        return render(request, "usercenter-mycourse.html", {
            'my_all_course': my_all_course
        })
...
class MyMessageView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        p = Paginator(user_message, 2, request=request)
